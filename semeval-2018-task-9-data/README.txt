...
-------
	...
	"scorer.py": official evaluation script (updated)

The official evaluation script for this task can be run from the terminal as follows:

	$ python scorer.py [gold_keys] [output_keys]

Example of usage:

	$ python scorer.py trial/gold/1A.english.trial.gold.txt trial/sample_output/1A.english.sample.output.txt
...
Note 3: We also include the old scorer, which contained a small bug which affected the P@K evaluation measures.
